export const getEnv = (env: any, k: string): string | undefined => {
  try { if (env?.[k]) return env[k]; } catch {}
  try { if ((process as any)?.env?.[k]) return (process as any).env[k]; } catch {}
  try { if ((import.meta.env as any)?.[k]) return (import.meta.env as any)[k]; } catch {}
  try { if ((globalThis as any)?.[k]) return (globalThis as any)[k]; } catch {}
  return undefined;
};
  ...
  while (attempt <= 1) {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        let wait = j?.retry_after ? (j.retry_after > 1000 ? j.retry_after : j.retry_after * 1000) : 1000;
