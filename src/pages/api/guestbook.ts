  ...
  try {
    ...
    const getEnv = (k: string) => {
      ...
      try {
        ...
        // @ts-ignore
        if (typeof globalThis !== "undefined" && (globalThis as any)[k])
    ...
    if (name.length > MAX_NAME) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/guestbook?status=error" },
      });
    }
    ...
    if (message.length > MAX_MESSAGE) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/guestbook?status=toolong" },
      });
    }
    ...
    async function processTurnstile(cf_turnstile_response: string) {
      ...
      if (!secret)
        return { success: false, errorCodes: ["missing-site-secret"] };
    ...
    if (!token) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/guestbook?status=turnstile" },
      });
    }
    ...
    if (isLocal) {
      ...
      if (!turnstileResult.success) {
        ...
        if (
          ...
          return new Response(null, {
            status: 303,
            headers: { Location: "/guestbook?status=turnstile_invalid_domain" },
          });
        }
        return new Response(null, {
          status: 303,
    ...
    // KV-based IP rate limit: prevent repeated guestbook submissions from same client
    try {
      const clientIp =
        request.headers.get("cf-connecting-ip") ??
        request.headers.get("x-forwarded-for") ??
        "anon";
      const throttleKey = `guestbook_throttle:${clientIp}`;
      if (
        env &&
        typeof env.SESSION !== "undefined" &&
        env.SESSION &&
        typeof env.SESSION.get === "function"
      ) {
        try {
          const existing = await env.SESSION.get(throttleKey);
          if (existing) {
            console.warn("Throttling guestbook from", clientIp);
            return new Response(null, {
              status: 303,
              headers: { Location: "/guestbook?status=error" },
            });
          }
          // 60s TTL — guestbook submissions should be less frequent than asks
          await env.SESSION.put(throttleKey, "1", { expirationTtl: 60 });
        } catch (e) {
    ...
    try {
      const webhookUrl = (getEnv("DISCORD_WEBHOOK_GUESTBOOK") ?? "")
        .toString()
        .trim();
      if (webhookUrl) {
        ...
        const mentionContent = mentionUserId
          ? `<@${mentionUserId}>`
          : undefined;
          ...
          while (attempt <= maxRetries) {
            ...
            try {
              ...
              if (res.status === 429) {
                ...
                try {
                  ...
                  if (j && typeof j.retry_after !== "undefined") {
                    ...
                    if (Number.isFinite(val))
                      retryAfterMs =
                        val > 1000 ? Math.ceil(val) : Math.ceil(val * 1000);
                  }
                } catch (e) {}
                ...
                if (!retryAfterMs) {
                  ...
                  if (Number.isFinite(num))
                    retryAfterMs =
                      num > 1000 ? Math.ceil(num) : Math.ceil(num * 1000);
                }
              ...
              if ((e as any)?.name === "AbortError") {
                console.warn(
                  "Discord webhook request aborted due to timeout (attempt",
                  attempt,
                  ")",
                );
              } else {
        ...
        console.info(
          "DISCORD_WEBHOOK_GUESTBOOK / DISCORD_WEBHOOK_URL not set — skipping webhook send",
        );
      }
    } catch (e) {
    ...
    if (typeof webhookFailed !== "undefined" && webhookFailed) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/guestbook?status=ok_webhook_failed" },
      });
    }

    return new Response(null, {
      status: 303,
      headers: { Location: "/guestbook?status=ok" },
    });
  } catch (err) {
