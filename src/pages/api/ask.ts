  ...
  try {
    ...
    if (name.length > MAX_NAME) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/ask?status=error" },
      });
    }
    ...
    if (message.length > MAX_MESSAGE) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/ask?status=toolong" },
      });
    }
    ...
    if (!token) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/ask?status=turnstile" },
      });
    }
    ...
    try {
      const webhookUrl = (getEnv("DISCORD_WEBHOOK_ASKS") ?? "")
        .toString()
        .trim();
      if (webhookUrl) {
          ...
          while (attempt <= maxRetries) {
            ...
            try {
              ...
              if ((e as any)?.name === "AbortError") {
                console.warn(
                  "Ask webhook aborted due to timeout (attempt",
                  attempt,
                  ")",
                );
              } else {
        ...
        console.info(
          "DISCORD_WEBHOOK_ASK / DISCORD_WEBHOOK_URL not set — skipping ask webhook send",
        );
      }
    } catch (e) {
    ...
    if (typeof webhookFailed !== "undefined" && webhookFailed) {
      return new Response(null, {
        status: 303,
        headers: { Location: "/ask?status=ok_webhook_failed" },
      });
    }

    return new Response(null, {
      status: 303,
      headers: { Location: "/ask?status=ok" },
    });
  } catch (err) {
